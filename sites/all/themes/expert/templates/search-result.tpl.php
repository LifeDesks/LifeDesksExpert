...
switch($result['type']) {
  case t('Taxon Page'):
    $tid = db_result(db_query("SELECT tn.tid FROM {term_node} tn INNER JOIN {node} n ON (tn.nid=n.nid) INNER JOIN {term_data} td ON (tn.tid = td.tid) WHERE tn.nid=%d AND td.vid=%d AND n.type='taxon_description'", $result['node']->nid, CLASSIFICATION_PRIMARY));
    if(!$tid) $hit = false;
  ...
  case 'Taxon Term':
    $italicize = (isset($result['node']->scientific) && $result['node']->scientific == 1) ? classification_check_italicize($result['node']->tid) : false;
    $result['title'] = ($italicize) ? $names->italicized_form($result['title']) : $result['title'];
  break;
}
?>
...
<dd>
	...
	<?php else : ?>
	  <p class="search-info"><?php print $result['link']; ?></p>
	<?php endif; ?>
</dd>
<?php endif; ?>
