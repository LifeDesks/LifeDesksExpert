...
var NameCellEditor = function($container, columnDef, value, dataContext) {
    var $input;
    var defaultValue = value;
    var scope = this;

    var vid = columnDef.cssClass.replace("vid-","");

    this.init = function() {

        $input = $("<INPUT type=text class='editor-text ac_input' />");
    ...
    this.validate = function() {
        if (columnDef.validator) 
        {
	        validator = window[columnDef.validator];
            var validationResults = validator(scope.getValue());
            if (!validationResults.valid) 
                return validationResults;
        }
...
var FlagCellEditor = function($container, columnDef, value, dataContext) {
    var $input;
    var defaultValue = value;
    var scope = this;
    ...
    this.init = function() {
        ...
        if (value != null) 
        {
            $input[0].defaultValue = value;
            $input.val(defaultValue);
            $.get(Drupal.settings.basePath + "classification_slickgrid_getflag/" + vid + "/" + defaultValue, {}, function(data) {
	          $input.data("extraData", {"flag_tid":data.flag_tid});
            }, "json");
        }
        
        $input.appendTo($container);
	    $input.autocomplete(
			Drupal.settings.basePath + "classification_slickgrid_autocomplete/" + vid + "/",
			{
				delay:1,
				minChars:1,
				matchSubset:1,
