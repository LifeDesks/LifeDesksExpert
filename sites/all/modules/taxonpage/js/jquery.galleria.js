...
$$.onPageLoad = function(_src) {	
	...
	if (_src) {
		...
		// define a new image
		_preview_name = _thumb.attr('name');
		_preview = _preview_name.split("||");
		
		var _img = $(new Image()).attr('src',escape(_preview[0])).addClass('replaced');

		// empty the wrapper and insert the new image
		_wrapper.empty().append(_img);

		if(_preview[1] !== '') {
		  _img.wrap("<a href='" + escape(_preview[1]) + "'></a>");
		}
